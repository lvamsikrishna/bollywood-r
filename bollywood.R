...
# Store the movie name in variable named top_2_movie
  ...
  top_2_movie <- bollywood$Movie[order(-bollywood$Tcollection)][2]
...
# Write command/s to find out how many movies are in Flop, Average, Hit and Superhit categories in the entire Bollywood data set.
  flop_count <- length(which(bollywood$Verdict == "Flop"))
  
  average_count <- length(which(bollywood$Verdict == "Average"))
  
  hit_count <- length(which(bollywood$Verdict == "Hit"))
  
  superhit_count <- length(which(bollywood$Verdict == "Super Hit"))
...
#You can write a command to find the maximum value of Ocollection, Wcollection, Fwcollecion and Tcollection using sapply
  ...
  max_collection <- sapply(bollywood[,c("Ocollection","Wcollection","Fwcollection","Tcollection")],max,na.rm=TRUE)
